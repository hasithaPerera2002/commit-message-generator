...
function activate(context) {
    let disposable = vscode.commands.registerCommand('commitGenerator.generate', async () => {
...
async function buildCommitStructure(changes) {
    const totalFiles = changes.modified.length +
...
async function selectCommitType(changes) {
    const types = [
...
function determineScope(changes) {
    const allFiles = [
        ...changes.modified,
        ...changes.added,
        ...changes.deleted,
    ...
    if (allFiles.every(f => /components?/i.test(f)))
        return 'components';
    if (allFiles.every(f => /api|routes?/i.test(f)))
        return 'api';
    if (allFiles.every(f => /utils?|helpers?/i.test(f)))
        return 'utils';
    if (allFiles.every(f => /types?/i.test(f)))
        return 'types';
    return undefined;
}
function generateSubject(changes, type, totalFiles) {
    // All new files
...
function generateBody(changes, totalFiles) {
    const config = vscode.workspace.getConfiguration('commitGenerator');
    const maxFilesInBody = config.get('maxFilesInBody', 20);
    const lines = [];
...
async function generateFooter() {
    const config = vscode.workspace.getConfiguration('commitGenerator');
    const includeFooter = config.get('includeFooter', false);
    if (!includeFooter) {
        return undefined;
    }
...
function formatCommitMessage(structure) {
    let message = '';
