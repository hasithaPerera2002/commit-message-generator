...
async function buildCommitStructure(
  changes: GitChanges
): Promise<CommitStructure | null> {
  const totalFiles =
...
async function selectCommitType(changes: GitChanges): Promise<string | null> {
  const types = [
    ...
    },
    {
      ...
      detail: "💎 Formatting, semicolons",
    },
    {
      label: "refactor",
      description: "Code refactoring",
      detail: "♻️ Code restructuring",
    },
    { label: "test", description: "Adding tests", detail: "🧪 Test coverage" },
    {
      label: "chore",
      description: "Maintenance",
      detail: "🔧 Build, dependencies",
    },
    {
      label: "perf",
  ...
  if (allFiles.some((f) => /test|spec|__tests__/i.test(f))) {
    ...
    allFiles.some((f) => f === "package.json" || f === "package-lock.json")
  ) {
    autoDetected = "chore";
  ...
  if (autoDetected) {
    ...
    if (detectedType) {
      typesToShow = [
        {
          ...detectedType,
          description: `${detectedType.description} (suggested)`,
        },
...
function determineScope(changes: GitChanges): string | undefined {
  ...
  const dirs = allFiles.map((f) => f.split("/")[0]).filter((d) => d !== "");
...
function generateSubject(
  ...
  const allFiles = [...changes.modified, ...changes.added, ...changes.deleted];
